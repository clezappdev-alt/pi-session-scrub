...
- [ ] T12 Document the new flow in `README.md` (WEAK-first page, compact default + `--verbose`, bulk machine confirm + fixed rationale, `id:name:"slug"` rename + duplicate/empty policy, deferred-ID line) as a SEPARATE meta commit, never mixed with the code commit. Maps: proposal §6, design §1. Verify: `git log --oneline -2` shows code + meta commits distinct. <!-- sdd-owner: implementation -->
